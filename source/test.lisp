...
(def function standalone-test-system/build-lisp-form (system-name system-version output-path &key (disable-debugger #t) (run-at (now)) (swank-directory "slime/"))
  ...
  `((in-package :cl-user)
    ...
    (eval-when (:compile-toplevel :load-toplevel :execute)
      (require :asdf))
    (asdf:initialize-source-registry '(:source-registry (:directory ,(merge-pathnames "hu.dwim.asdf/" hu.dwim.home::*workspace-directory*))
                                       :ignore-inherited-configuration))
    (asdf:initialize-output-translations '(:output-translations
                                           (,(namestring hu.dwim.home::*workspace-directory*) ,(namestring hu.dwim.home::output-path))
                                           ("/opt/darcs/" ,(namestring hu.dwim.home::output-path))
                                           :ignore-inherited-configuration))
    ;; another option, but more fragile to changes and also has trouble with the .fasl dir... (load ,(merge-pathnames "hu.dwim.environment/source/environment.lisp" hu.dwim.home::*workspace-directory*))
    ...
    ,@(when (eq hu.dwim.home::system-version :head)
        ...
        `((initialize-asdf-source-registry '(#P"/opt/darcs/" ,hu.dwim.home::*workspace-directory*))))
